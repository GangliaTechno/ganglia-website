...
export default function ApplicationForm() {
  ...
  // Updated email body with timestamp and name
  const composeEmailBody = (timestamp) => {
    return `
      <div style="font-family: Arial, sans-serif; max-width: 600px;">
        ...
        <div style="background-color: #f8f9fa; padding: 15px; border-radius: 5px; margin-bottom: 20px; border-left: 4px solid #3498db;">
          <p style="margin: 5px 0;"><strong>📅 Submission Time:</strong> ${timestamp}</p>
          <p style="margin: 5px 0;"><strong>👤 Applicant Name:</strong> ${form.fullName}</p>
          <p style="margin: 5px 0;"><strong>📧 Applicant Email:</strong> ${form.email}</p>
          <p style="margin: 5px 0;"><strong>📍 Position:</strong> ${(state?.jobTitle ?? form.roleAppliedFor) || "N/A"}</p>
        </div>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append(
        "subject", 
        `New Application: ${form.fullName} - ${(state?.jobTitle ?? form.roleAppliedFor) || "Position"} (${timestamp})`
      );
      
      // Pass timestamp to email body
      formData.append("body", composeEmailBody(timestamp));
